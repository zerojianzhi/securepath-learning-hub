(function () {
  ...
  function renderPractice() {
    return `<main class="practice-page"><div class="workspace-heading"><div><span class="eyebrow">AUTHORIZED LABS · PRACTICE</span><h1>实战任务与推荐靶场</h1><p>每个任务都有明确成果；靶场入口均指向官方项目或授权训练平台。</p></div><button class="button button-primary button-small" data-action="go-need" data-node-id="virtual-lab">先搭建实验环境</button></div><section class="lab-section"><div class="section-heading"><div><h2>推荐靶场</h2><p>${DATA.LABS.length} 个已收录入口，按方向、费用与部署方式选择；点击可直接打开官方页面。</p></div></div><div class="lab-grid">${DATA.LABS.map(renderLabCard).join("")}</div><div class="lab-safety-banner">⚠ 所有本地靶场请使用 VirtualBox/VMware 的 NAT 或 Host-only 网络，保留快照；不要将 DVWA、Vulhub、Metasploitable 等易受攻击环境暴露到公网。</div></section><section class="task-section"><div class="section-heading"><div><h2>可提交的实战任务</h2><p>完成靶场题后，把范围、过程、证据和修复建议写入学习笔记。</p></div></div><div class="practice-grid">${DATA.PRACTICES.map((practice) => `<article class="practice-card"><div class="eyebrow">${stageById(practice.stage).short} · ${practice.level}</div><h3>${practice.title}</h3><p>完成标准：${practice.outcome}</p><div class="practice-outcome">产出：${practice.outcome}</div><div class="safety-note">⚠ ${practice.safety}</div><button class="status-button ${state.completedPractices[practice.id] ? "done" : ""}" data-action="toggle-practice" data-practice-id="${practice.id}"><span class="status-dot"></span>${state.completedPractices[practice.id] ? "已完成任务" : "标记为完成"}</button></article>`).join("")}</div></section></main>`;
  }

  function renderLabCard(lab) {
    const stage = stageById(lab.stage);
    const node = nodeById(lab.nodeId);
    return `<article class="lab-card"><div class="lab-card-top"><span class="lab-category">${escapeHtml(lab.category)}</span>${pricingBadge(lab.pricing)}</div><h3>${escapeHtml(lab.title)}</h3><p>${escapeHtml(lab.summary)}</p><div class="lab-meta"><span>${escapeHtml(lab.lang)}</span><span>${escapeHtml(lab.deployment)}</span><span>${escapeHtml(lab.access)}</span></div><div class="lab-path">对应路线：${stage?.short || "实践"} · ${escapeHtml(node?.title || "综合练习")}</div><div class="lab-links"><a class="button button-primary button-small" href="${safeExternalUrl(lab.url)}" target="_blank" rel="noreferrer">进入靶场 ↗</a><button class="button button-ghost button-small" data-action="go-need" data-node-id="${lab.nodeId}">查看学习节点</button></div><div class="safety-note">⚠ ${escapeHtml(lab.safety)}</div></article>`;
  }
